...
这里补充一个MySQL可能会用到的操作，例如我使用的是国外的服务器，国内访问数据库延迟较高，在Navicat中备份数据库结构的时候会很慢，因为Navicat是依次获取每张表的表结构对延迟要求较高影响效率，这里我选择直接在远程服务器中使用`dump`命令将库备份到服务器中，然后使用FPT工具将备份好的SQL文件下载下来即可，单个文件的下载相比多次获取表结构的效率要高得多，这里记录一下操作命令
